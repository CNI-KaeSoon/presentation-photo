...
4. **PDF 만들기** — 폴더별, 통합, 순서 지정 통합 중 필요한 방식을 골라 `03_결과물`에 저장합니다.

작업 순서 패널의 고해상도 PDF는 로컬 Python으로 만들기 때문에 PDF 생성에 인터넷이 필요 없고 사진도 외부로 전송하지 않습니다. 보정 툴바에는 상태 JSON을 저장·복원하는 `백업 내보내기`와 `백업 불러오기`만 남아 있습니다.
